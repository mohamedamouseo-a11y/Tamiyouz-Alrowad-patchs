...
PATCH_ID = "JOIN_US_REFERENCE_EXACT_V3_5"
PATCH_MARKER = "TAM_JOIN_US_REFERENCE_EXACT_V3"
HOTFIX_MARKER = "TAM_JOIN_US_V3_5_LINEAGE_HOTFIX"
...
def load_base(base_script: Path):
    ...
    ns = {
        "__name__": "tamiyouz_v3_base_hotfixed_v35",
...
def analyze_source_lineage(source: str):
    counts = {
        "v1_total_before": source.count(V1_MARKER),
        "v2_total_before": source.count(V2_MARKER),
    }
    if counts["v1_total_before"] != 1:
        raise SystemExit(
            f"Expected exactly one {V1_MARKER} in live source, found {counts['v1_total_before']}"
        )
    if counts["v2_total_before"] != 1:
        raise SystemExit(
            f"Expected exactly one {V2_MARKER} in live source, found {counts['v2_total_before']}"
        )
    ...
    old_main = matches[0].group(0)
    lineage = {
        "v1_inside_replaced_main": V1_MARKER in old_main,
        "v2_inside_replaced_main": V2_MARKER in old_main,
        "v1_outside_replaced_main": V1_MARKER in (source[:matches[0].start()] + source[matches[0].end():]),
        "v2_outside_replaced_main": V2_MARKER in (source[:matches[0].start()] + source[matches[0].end():]),
    }

    if lineage["v1_inside_replaced_main"] == lineage["v1_outside_replaced_main"]:
        raise SystemExit("V1 marker location is ambiguous; expected exactly one location")
    if lineage["v2_inside_replaced_main"] == lineage["v2_outside_replaced_main"]:
        raise SystemExit("V2 marker location is ambiguous; expected exactly one location")
...
def patch_runtime_main(ns, lineage):
    ...
    if HOTFIX_MARKER in base_main:
        raise SystemExit("V3.5 hotfix marker already present in original V3 MAIN")

    injected = []
    if lineage["v1_inside_replaced_main"]:
        injected.append(f"<!-- {V1_MARKER} -->")
    if lineage["v2_inside_replaced_main"]:
    ...
    if patched_main.count(HOTFIX_MARKER) != 1:
        raise SystemExit("V3.5 hotfix marker count is not exactly 1 in patched runtime MAIN")

    expected_v1_in_main = 1 if lineage["v1_inside_replaced_main"] else 0
    expected_v2_in_main = 1 if lineage["v2_inside_replaced_main"] else 0
    if patched_main.count(V1_MARKER) != expected_v1_in_main:
        raise SystemExit("V1 runtime MAIN injection count mismatch")
    if patched_main.count(V2_MARKER) != expected_v2_in_main:
        raise SystemExit("V2 runtime MAIN injection count mismatch")

    return {
        "v1_reinjected": lineage["v1_inside_replaced_main"],
        "v2_reinjected": lineage["v2_inside_replaced_main"],
        "v1_preserved_outside": lineage["v1_outside_replaced_main"],
        "v2_preserved_outside": lineage["v2_outside_replaced_main"],
    }
...
def main():
    ...
    if failed:
        raise SystemExit("V3.5 post-transform guard failed: " + ", ".join(failed))
    ...
    manifest = {
        ...
        "bugs_fixed": [
            "Determine V1/V2 marker locations relative to the exact legacy main replaced by original V3.",
            "Reinject only lineage markers that would actually be removed by the original V3 main replacement.",
            "Preserve lineage markers already outside the replaced main without duplicating them.",
        ...
        "base_main_patch_mode": "runtime_namespace_conditional_lineage",
