#!/usr/bin/env python3
import argparse
import hashlib
import json
import re
from pathlib import Path

PATCH_ID = "JOIN_US_FORM_FUNCTIONAL_V3_16"
PATCH_MARKER = "TAM_JOIN_US_V3_16_FORM_FUNCTIONAL"
STYLE_ID = "tamiyouz-join-us-v316-form-functional"

REQUIRED = [
    "TAM_JOIN_US_REFERENCE_EXACT_V3",
    "TAM_JOIN_US_V3_9_MEDIA_FALLBACK",
    'id="careersFrame"',
    "ref3-form-shell",
    "ref3-form-mask",
    "https://careers.tamiyouzplaform.com/",
    "https://careers.tamiyouzplaform.com/api/v1/rakan/chat",
    "fetch('/join-us/notify.php'",
    "e.data.type === 'formSubmission'",
]

OVERRIDE = r'''<style id="tamiyouz-join-us-v316-form-functional">
/* TAM_JOIN_US_V3_16_FORM_FUNCTIONAL */

/*
 V3.16 priority: make the application usable first.
 - Remove all parent-page cropping/shields introduced by visual fixes.
 - Restore natural iframe width and pointer interaction.
 - Show a large, scrollable iframe from its real top position.
 - Keep page-level support widgets untouched.
*/
.ref3-application{
  max-width:1560px!important;
  grid-template-columns:180px minmax(760px,980px) 180px!important;
  gap:28px!important;
  align-items:start!important;
  padding-inline:24px!important;
}
.ref3-form-shell{
  width:100%!important;
  max-width:none!important;
  overflow:visible!important;
  isolation:auto!important;
}
.ref3-form-mask{
  position:relative!important;
  width:100%!important;
  height:1800px!important;
  min-height:1800px!important;
  overflow:auto!important;
  background:#0b1b2b!important;
}
.ref3-form-mask iframe#careersFrame{
  display:block!important;
  position:relative!important;
  z-index:1!important;
  left:auto!important;
  right:auto!important;
  width:100%!important;
  max-width:100%!important;
  height:1800px!important;
  min-height:1800px!important;
  margin:0!important;
  transform:none!important;
  border:0!important;
  background:#0b1b2b!important;
  pointer-events:auto!important;
  overflow:auto!important;
}

/* Neutralize every local iframe shield/mask from V3.14/V3.15. */
.ref3-form-mask::after{
  content:none!important;
  display:none!important;
  pointer-events:none!important;
}
.ref3-iframe-widget-shield{
  display:none!important;
  pointer-events:none!important;
  width:0!important;
  height:0!important;
}

@media(max-width:1180px){
  .ref3-application{grid-template-columns:140px minmax(680px,820px) 140px!important;gap:18px!important}
}
@media(max-width:980px){
  .ref3-application{grid-template-columns:1fr!important;max-width:860px!important}
  .ref3-form-mask{height:1850px!important;min-height:1850px!important}
  .ref3-form-mask iframe#careersFrame{height:1850px!important;min-height:1850px!important}
}
@media(max-width:620px){
  .ref3-application{padding-inline:10px!important}
  .ref3-form-mask{height:1900px!important;min-height:1900px!important}
  .ref3-form-mask iframe#careersFrame{height:1900px!important;min-height:1900px!important}
}
</style>'''

IFRAME_OPEN_RE = re.compile(r'<iframe\b(?=[^>]*\bid=["\']careersFrame["\'])[^>]*>', re.I | re.S)


def make_iframe_scrollable(source: str):
    matches = list(IFRAME_OPEN_RE.finditer(source))
    if len(matches) != 1:
        raise SystemExit(f"Expected exactly one careersFrame opening tag, found {len(matches)}")
    old = matches[0].group(0)
    new = old
    if re.search(r'\bscrolling\s*=\s*["\'][^"\']*["\']', new, re.I):
        new = re.sub(r'\bscrolling\s*=\s*["\'][^"\']*["\']', 'scrolling="yes"', new, count=1, flags=re.I)
    else:
        new = new[:-1] + ' scrolling="yes">'
    if old == new:
        return source, old, new
    return source[:matches[0].start()] + new + source[matches[0].end():], old, new


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--source", required=True)
    ap.add_argument("--output", required=True)
    ap.add_argument("--manifest")
    args = ap.parse_args()

    source = Path(args.source).read_text(encoding="utf-8")
    for marker in REQUIRED:
        if marker not in source:
            raise SystemExit(f"Required live marker missing: {marker}")
    if PATCH_MARKER in source or STYLE_ID in source:
        raise SystemExit("V3.16 already present in source")
    if source.count('id="careersFrame"') != 1:
        raise SystemExit(f"Expected exactly one careersFrame, found {source.count('id=\"careersFrame\"')}")
    if "</head>" not in source:
        raise SystemExit("Missing </head> anchor")

    result, iframe_before, iframe_after = make_iframe_scrollable(source)
    result = result.replace("</head>", OVERRIDE + "\n</head>", 1)

    guards = {
        "patch_marker_once": result.count(PATCH_MARKER) == 1,
        "style_id_once": result.count(STYLE_ID) == 1,
        "careers_iframe_once": result.count('id="careersFrame"') == 1,
        "iframe_scrolling_enabled": 'scrolling="yes"' in iframe_after,
        "natural_iframe_width": "width:100%!important;" in OVERRIDE and "max-width:100%!important;" in OVERRIDE,
        "no_negative_margin": "margin:0!important;" in OVERRIDE,
        "no_transform": "transform:none!important;" in OVERRIDE,
        "pointer_interaction_restored": "pointer-events:auto!important;" in OVERRIDE,
        "mask_scrollable": "overflow:auto!important;" in OVERRIDE,
        "old_pseudo_shield_disabled": ".ref3-form-mask::after" in OVERRIDE and "content:none!important;" in OVERRIDE,
        "real_shield_disabled": ".ref3-iframe-widget-shield" in OVERRIDE and "display:none!important;" in OVERRIDE,
        "careers_preserved": "https://careers.tamiyouzplaform.com/" in result,
        "rakan_preserved": "https://careers.tamiyouzplaform.com/api/v1/rakan/chat" in result,
        "form_submission_preserved": "e.data.type === 'formSubmission'" in result,
        "notify_preserved": "fetch('/join-us/notify.php'" in result,
        "v3_preserved": "TAM_JOIN_US_REFERENCE_EXACT_V3" in result,
        "v39_preserved": "TAM_JOIN_US_V3_9_MEDIA_FALLBACK" in result,
        "visible_h1_once": result.count('<h1 id="join-us-title">') == 1,
        "no_global_rakan_hide": ".rakan" not in OVERRIDE,
        "no_global_whatsapp_hide": ".whatsapp" not in OVERRIDE,
    }
    failed = [k for k, v in guards.items() if not v]
    if failed:
        raise SystemExit("V3.16 guard failed: " + ", ".join(failed))

    Path(args.output).write_text(result, encoding="utf-8")
    manifest = {
        "patch": PATCH_ID,
        "patch_marker": PATCH_MARKER,
        "style_id": STYLE_ID,
        "strategy": "functional_first_natural_iframe_scrollable_no_shields",
        "iframe_before": iframe_before,
        "iframe_after": iframe_after,
        "source_sha256": hashlib.sha256(source.encode("utf-8")).hexdigest(),
        "output_sha256": hashlib.sha256(result.encode("utf-8")).hexdigest(),
        "files_changed": ["themes/thegem-elementor/page-join-us.php"],
        "guards": guards,
        "functional_qa_required": True,
        "visual_qa_required": True,
        "git_mutations_by_patch_runner": False,
    }
    if args.manifest:
        Path(args.manifest).write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    print(json.dumps(manifest, indent=2, ensure_ascii=False))


if __name__ == "__main__":
    main()
