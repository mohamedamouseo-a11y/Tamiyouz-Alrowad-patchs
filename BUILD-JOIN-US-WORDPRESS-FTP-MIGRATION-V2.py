...
def main():
    ap = argparse.ArgumentParser(description="Build an FTP-safe Join Us -> WordPress migration package from the CURRENT live index.html downloaded via FTP.")
    ...
    if len(raw) < 35000:
        raise SystemExit(f"Refusing unexpectedly small index.html: {len(raw)} bytes")
    ...
    helper = f'''<?php
// Tamiyouz Join Us one-time migration bootstrap. DELETE immediately after migration.
...
if ($action === 'prepare') {{
    ...
    add_option($stateKey, $state, '', false);
...
if ($action === 'rollback') {{
    ...
    if (!empty($state['had_existing_page']) && !empty($state['before']['ID'])) {{
        ...
        $r = wp_update_post(['ID' => $id, 'post_status' => $b['post_status'], 'post_name' => $b['post_name']], true);
...
'''
    ...
    manifest = {
        "builder": "BUILD-JOIN-US-WORDPRESS-FTP-MIGRATION-V2.py",
