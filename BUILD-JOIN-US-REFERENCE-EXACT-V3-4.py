...
PATCH_ID = "JOIN_US_REFERENCE_EXACT_V3_4"
PATCH_MARKER = "TAM_JOIN_US_REFERENCE_EXACT_V3"
HOTFIX_MARKER = "TAM_JOIN_US_V3_4_RUNTIME_HOTFIX"
...
LINEAGE_BLOCK = (
    ...
    "  <!-- TAM_JOIN_US_V3_4_RUNTIME_HOTFIX -->"
)
...
def load_fixed_base(base_script: Path):
    ...
    ns = {
        "__name__": "tamiyouz_v3_base_hotfixed_v34",
    ...
    if HOTFIX_MARKER in base_main:
        raise SystemExit("V3.4 hotfix marker already present in original V3 MAIN")
    ...
    if patched_main.count(HOTFIX_MARKER) != 1:
        raise SystemExit("V3.4 hotfix marker count is not exactly 1 in patched runtime MAIN")
...
def main():
    ...
    if failed:
        raise SystemExit("V3.4 post-transform guard failed: " + ", ".join(failed))
    ...
    manifest = {
        ...
        "bugs_fixed": [
            ...
            "Use a hotfix marker that does not contain the original V3 patch marker substring, so the original V3 marker_once guard remains true.",
