...
SIGNATURE = "TAM_JOIN_US_WORDPRESS_V4"
...
def main():
    ap = argparse.ArgumentParser(description="Build FTP/HTTP Join Us -> WordPress migration package V4 using page-join-us.php template hierarchy.")
    ...
    page_path = out / PAGE_FILE
    page_path.write_bytes(page_php)

    token = args.token or secrets.token_urlsafe(48)
    nonce = secrets.token_hex(8)
    helper_name = f"join-us-wp-bootstrap-v4-{nonce}.php"
    state_key = f"_tamiyouz_join_us_migration_v4_{nonce}"
...
function ju_pages() {{
    return get_posts([
        ...
        'numberposts' => 10, 'orderby' => 'ID', 'order' => 'ASC', 'suppress_filters' => false,
    ]);
}}
function ju_page() {{
    $posts = ju_pages();
...
'''
    ...
    manifest = {
        "builder": "BUILD-JOIN-US-WORDPRESS-FTP-MIGRATION-V4.py",
        ...
        "intentional_changes": [
            "Use slug-specific WordPress template hierarchy file page-join-us.php",
            "Replace exactly one /join-us/video-header.mp4?v=6 URL with theme asset URL",
            "Add inert HTML migration signature comment",
        ],
