...
PATCH_ID = "JOIN_US_REFERENCE_EXACT_V3_3"
PATCH_MARKER = "TAM_JOIN_US_REFERENCE_EXACT_V3"
HOTFIX_MARKER = "TAM_JOIN_US_REFERENCE_EXACT_V3_3_HOTFIX"
...
LINEAGE_BLOCK = (
    ...
    "  <!-- TAM_JOIN_US_REFERENCE_EXACT_V3_3_HOTFIX -->"
)
...
def load_fixed_base(base_script: Path):
    ...
    ns = {
        "__name__": "tamiyouz_v3_base_hotfixed_v33",
    ...
    base_main = ns["MAIN"]
    marker_count = base_main.count(BASE_MAIN_MARKER)
    if marker_count != 1:
        raise SystemExit(f"Expected exactly one V3 marker in base MAIN, found {marker_count}")
    if "TAM_JOIN_US_LUXURY_V1" in base_main or "TAM_JOIN_US_LUXURY_V2_FIDELITY" in base_main:
        raise SystemExit("Unexpected lineage marker already present in original V3 MAIN")
    if HOTFIX_MARKER in base_main:
        raise SystemExit("V3.3 hotfix marker already present in original V3 MAIN")

    ns["MAIN"] = base_main.replace(BASE_MAIN_MARKER, LINEAGE_BLOCK, 1)

    patched_main = ns["MAIN"]
    if patched_main.count(PATCH_MARKER) != 1:
        raise SystemExit("V3 marker duplication detected in patched runtime MAIN")
    if patched_main.count("TAM_JOIN_US_LUXURY_V1") != 1:
        raise SystemExit("V1 lineage marker count is not exactly 1 in patched runtime MAIN")
    if patched_main.count("TAM_JOIN_US_LUXURY_V2_FIDELITY") != 1:
        raise SystemExit("V2 lineage marker count is not exactly 1 in patched runtime MAIN")
    if patched_main.count(HOTFIX_MARKER) != 1:
        raise SystemExit("V3.3 hotfix marker count is not exactly 1 in patched runtime MAIN")
...
def main():
    ...
    final_guards = {
        "v1_marker_once": result.count("TAM_JOIN_US_LUXURY_V1") == 1,
        "v2_marker_once": result.count("TAM_JOIN_US_LUXURY_V2_FIDELITY") == 1,
    ...
    if failed:
        raise SystemExit("V3.3 post-transform guard failed: " + ", ".join(failed))
    ...
    manifest = {
        ...
        "bugs_fixed": [
            "Preserve V1/V2 lineage markers by patching the original V3 MAIN value in the executed runtime namespace.",
            "Keep the original V3 marker exactly once instead of reinserting it through source-prefix rewriting.",
            "Verify original V3 independently with raw file SHA256 and canonical Git blob SHA1.",
        ],
        "base_main_patch_mode": "runtime_namespace_only",
